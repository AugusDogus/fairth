...
export async function createAuthService(config: Config) {
  ...
  const secret = process.env.BETTER_AUTH_SECRET ?? await persistentSecret(join(config.authDataRoot, secretFilename));
  ...
  function ownerSetupUrl(): string | undefined {
    ...
    const url = new URL("/owner/setup", config.publicBaseUrl);
